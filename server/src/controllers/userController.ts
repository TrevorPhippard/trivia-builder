...
export const registerUser = async (account_id: number, user_name: string, email: string, password: string) => {
...
export const updateUserById = async (userId: number, id: number, user_name: string, role: string) => {
  const user = await User.findByPk(userId);
  if (!user) {
    return { status: 404, message: 'User not found' };
  } else {
